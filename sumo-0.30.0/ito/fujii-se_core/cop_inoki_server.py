...
class ParkingServer(BaseServer):
    ...
    def parking_assignment(self, sock, need_parking):
        if need_parking == ROTARY:
            ...
            floor = l.floor_select(self.countid1, self.vehcountid1, CAPACITY, ROTARY_sensor)
            if floor == 4:
                # ROTARYが満車の時
                ...
                parking = 'CENTER'
                floor = l.floor_select(self.countid2, self.vehcountid2, CAPACITY, CENTER_sensor)
                if floor == 4:
                    # CENTERが満車の時
                    ...
                    parking = 'WEST'
                    floor = l.floor_select(self.countid3, self.vehcountid3, CAPACITY, WEST_sensor)
    ...
    def deal_msg(self, sock, msg, readfds):
        ...
        ###################################################################
            ...
            # 満車確認、満車時対応
            if 'NotUser' in msg:
                #ここ修正すればいける、notuserレイヤーのアクティベートをそれぞれのifの中でよべばおk、引数変えて
                #非ユーザのfullcheck
                #非ユーザのfullchangecountはカウントするべきではない
                if 'ROTARY' in msg:
                    with notuser_layer.NotUser(self.veh_id1, self.vehcountid1, CAPACITY, ROTARY_sensor):
                        parking = 'ROTARY'
                    ...
                    with notuser_layer.NotUser(self.veh_id2, self.vehcountid2, CAPACITY, CENTER_sensor):
                        parking = 'CENTER'
                        floor =  l.floor_select()
                        if floor == 4:
                            parking = 'ROTARY'
                    ...
                    with notuser_layer.NotUser(self.veh_id3, self.vehcountid3, CAPACITY, WEST_sensor):
                        parking = 'WEST'
                        floor =  l.floor_select()
                        if floor == 4:
                            parking = 'ROTARY'
            ...
            #以下、ユーザのfullcheck
                ...
                if OK_check == False:
                    ...
                    if floor == 4:
                        ...
                        floor = l.floor_select(self.countid2, self.veh_id2, CAPACITY, CENTER_sensor)
                        parking = 'CENTER'
                        if floor == 4:
                            floor = l.floor_select(self.countid3, self.veh_id3, CAPACITY, WEST_sensor)
                            parking = 'WEST'
                            if floor == 4:
                                parking = 'GO_HOME'
                                self.assigned_driver-=1
                ...
                if OK_check == False:
                    ...
                    if floor == 4:
                        floor = l.floor_select(self.countid1, self.veh_id1, CAPACITY, ROTARY_sensor)
                        parking = 'ROTARY'
                        if floor == 4:
                            floor = l.floor_select(self.countid3, self.veh_id3, CAPACITY, WEST_sensor)
                            parking = 'WEST'
