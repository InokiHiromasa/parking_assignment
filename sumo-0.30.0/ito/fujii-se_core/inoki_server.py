...
class ParkingServer(BaseServer):
    ...
    def floor_select(self, countid, vehnumber, capacity):
    ...
    def NotUser_floor_select(self, veh_id, capacity, sensor, vehcountid):
        # NotUserが駐車場に到着した際に使う
        # vehidのみを考慮
        # vehidの増加に影響
        # sensorは、文字列判定のため文字列として入力してもらう
        floor = 1
        veh_id[0]+=1
    ...
    def parking_assignment(self, sock, need_parking):
        if need_parking == ROTARY:
            ...
            floor = self.floor_select(self.countid1, self.vehcountid1, CAPACITY)
            if floor == 4:
                # ROTARYが満車の時
                ...
                parking = 'CENTER'
                floor = self.floor_select(self.countid2, self.vehcountid2, CAPACITY)
                if floor == 4:
                    # CENTERが満車の時
                    ...
                    parking = 'WEST'
                    floor = self.floor_select(self.countid3, self.vehcountid3, CAPACITY)
    ...
    def deal_msg(self, sock, msg, readfds):
        ...
        # 駐車場管理
            ...
            if 'reach_ROTARY' in msg:
                ...
                #出て来た奴がアプリユーザである確率　= vehcountid / veh_id で，出て来た奴がアプリユーザかどうか決めよう
            ...
            #以下、ユーザのfullcheck
                ...
                if OK_check == False:
                    ...
                    if floor == 4:
                        ...
                        floor = self.floor_select(self.countid2, self.veh_id2, CAPACITY)
                        parking = 'CENTER'
                        if floor == 4:
                            floor = self.floor_select(self.countid3, self.veh_id3, CAPACITY)
                            parking = 'WEST'
                            if floor == 4:
                                parking = 'GO_HOME'
                                self.assigned_driver-=1
                ...
                if OK_check == False:
                    ...
                    if floor == 4:
                        floor = self.floor_select(self.countid1, self.veh_id1, CAPACITY)
                        parking = 'ROTARY'
                        if floor == 4:
                            floor = self.floor_select(self.countid3, self.veh_id3, CAPACITY)
                            parking = 'WEST'
